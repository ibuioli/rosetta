...
export class P52OfService {
  ...
  public conversor(p5: string, of: string): string {
    ...
    if (of === '0.9.x') {
      ...
      const c_point = this.t.repeted(r_p5, 'point(', false);
      for (let i = 0; i < c_point; i++) {
        const m = this.t.extract(r_p5, 'point(', ');');
        const value = 'ofDrawRectangle(' + m + ', 1, 1);';
    ...
    if (r_p5.indexOf('textFont(') === -1) {
      ...
      const pfont = [];
      const c_tfont = this.t.repeted(r_p5, 'textFont(', false);
      for (let i = 0; i < c_tfont; i++) {
        const m = this.t.extract(r_p5, 'textFont(', ');');
        pfont[i] = m;
    ...
    /////// GRAPHICS ///////
    const c_cgraphics = this.t.repeted(r_p5, '=createGraphics(', false);
    for (let i = 0; i < c_cgraphics; i++) {
      const m = this.t.extract(r_p5, '=createGraphics(', ');');
      const mc = this.t.countpar(m, ',');
      const p = this.t.params(m, ',', mc + 1);
  ...
  public ofApph(p5: string): string {
    ...
    const csetup = p5.search('void setup()');
    const cdraw = p5.search('void draw()');

    if (csetup !== -1 || cdraw !== -1) {
      const keyPressed = p5.search(/void keyPressed\(\)/);
      const keyReleased = p5.search(/void keyReleased\(\)/);
      const mousePressed = p5.search(/void mousePressed\(\)/);
      const mouseReleased = p5.search(/void mouseReleased\(\)/);
      const mouseMoved = p5.search(/void mouseMoved\(\)/);
  ...
  public ofAppcpp(p5: string): string {
    ...
    const csetup = r_p5.search('void setup()');
    const cdraw = r_p5.search('void draw()');

    const csize = this.t.repeted(r_p5, 'ofSetupOpenGL(', false);
    for (let i = 0; i < csize; i++) {
      const m = this.t.extract(r_p5, 'ofSetupOpenGL(', ');');
      const value = '';
  ...
  public maincpp(p5: string): string {
    ...
    const csize = this.t.repeted(r_p5, 'ofSetupOpenGL(', false);
    const cfull = this.t.repeted(r_p5, 'fullScreen(', false);
    if (csize === 1) {
      const m = this.t.extract(r_p5, 'ofSetupOpenGL(', ')');
      const ms = m.split(',');
      if (ms.length === 3) {
    ...
    const maincpp = '#include "ofMain.h"\n#include "ofApp.h"\n\nint main( ){\n\n\t' + opengl + '\n\n\tofRunApp(new ofApp());\n}';
  ...
  public p5ver(p5: string): string {
    ...
    const c_rect = this.t.repeted(p5, 'rect(', false);
    let rect = false;
    for (let i = 0; i < c_rect; i++) {
      const m = this.t.extract(p5, 'rect(', ');');
      if (this.t.countpar(m, ',') <= 3) {
        rect = false;
      } else if (this.t.countpar(m, ',') > 3) {
        rect = true;
